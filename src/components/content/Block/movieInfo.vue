...
<div>    
    <div class="videoTitle">
        <div class="videoInfo" >{{movieInfo.title}}</div> 
        <div class="afollow" @click="followAnime" v-if="follow">♥ 追剧</div>
        <div class="afollow followed" @click="followAnime" v-else>已追剧</div>
        <div class="videoData">
            <div><i class="el-icon-video-play"></i></div>
            <div><i class="el-icon-postcard"></i></div>
            <div><span>{{movieInfo.type}}</span></div>
            <div><span></span></div>
        </div>
    </div>
    <likeBar>
...
export default {
    data(){
        return{
            follow: this.$store.state.movieDataAll.follow,
            movieFollow:0,
            touch:this.$store.state.movieDataAll.touch,
        }
    ...
    methods:{
        followAnime(){
            ...
            this.$store.commit('movieFollow') 
            this.follow = !this.follow
            if(this.follow == false){
                this.$store.commit('addToFollowMovie',this.movieInfo)
            }else{
                this.$store.commit('deleteFollowMovie',this.$route.params.av)
            }
        ...
        touchLike(){
            ...
            this.$store.commit('movieTouch')

        },
    },
    beforeUpdate(){
        this.touch = this.$store.state.movieDataAll.touch
        this.follow = this.$store.state.movieDataAll.follow
    },
    activated(){ 
    },
    // created(){
